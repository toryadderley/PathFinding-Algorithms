...
class Model {
	...
	// 0 <= x < MAP_WIDTH.
	// 0 <= y < MAP_HEIGHT.
	// Call getTravelSpeed for AI to be aware of travel view
	...
	float getX() {
		return sprites.get(0).x;
	}

	float getY() {
		return sprites.get(0).y;
	}

	float getDestinationX() {
		return sprites.get(0).xDestination;
	}

	float getDestinationY() {
		return sprites.get(0).yDestination;
	}

	// Tells agent where to go
	...
	class Sprite {
		...
		// Moves the agent
		void update() {
			// Tells the agent the speed to move based on location on terrain
